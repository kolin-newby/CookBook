...
const RecipeDetails = ({
  ...
  return (
    <div
      className={`absolute inset-0 transform translate-x-full z-10 flex flex-col items-center justify-center h-full w-full text-lg font-bold bg-theme-2 p-8 inset-shadow-sm text-yellow-950 space-y-3`}
    >
      {recipe && (
        <>
          {showModal && (
            <RecipeModal
              ...
              onClose={() => {
                setOpen(false);
                setTimeout(() => setEditRecipe(null), 700);
              }}
            />
          )}
          <div className="flex flex-col items-center max-w-[800px] w-full h-full space-y-1.5">
            {showManageView && (
              <MutationBar
                recipe={recipe}
            ...
            <div className="flex w-full space-x-4 items-center">
              <div
                className={`relative aspect-square h-16 items-center justify-center group cursor-pointer`}
                onClick={handleClose}
              >
                ...
                <div className="absolute flex items-center justify-center inset-0 w-full h-full">
                  <MoveLeftIcon className="text-theme-2" size={"25"} />
                </div>
              </div>
              <div className="flex flex-col space-y-2 justify-center">
                <span className="flex flex-row">
                  <h2 className="flex h-full w-full text-2xl md:text-4xl items-center">
                    {recipe?.title}
                  </h2>
                </span>
            ...
            <TabGroup
              className={
                "flex flex-col pt-4 items-center h-full w-full min-h-0"
              }
            >
              <TabList
                className={
